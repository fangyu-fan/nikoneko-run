@preconcurrency import ActivityKit
import Foundation
import Observation

@Observable
@MainActor
final class LiveActivityService {
    private var activity: Activity<NikoNekoLiveActivityAttributes>?
    private var sessionStart: Date = Date()
    private var updateTask: Task<Void, Never>?

    func start(bpm: Int, target: TimeInterval, characterId: String, themeId: String) {
        guard ActivityAuthorizationInfo().areActivitiesEnabled else { return }
        end()
        sessionStart = Date()
        let state = NikoNekoLiveActivityAttributes.ContentState(
            elapsed: 0, remaining: target, bpm: bpm,
            characterId: characterId, themeId: themeId, isCountdown: target > 0,
            hr: 0, distance: 0, steps: 0,
            startDate: sessionStart,
            targetDuration: target
        )
        let content = ActivityContent(state: state, staleDate: Date().addingTimeInterval(6 * 3600))
        activity = try? Activity.request(
            attributes: NikoNekoLiveActivityAttributes(),
            content: content,
            pushType: nil
        )
    }

    func update(elapsed: TimeInterval, remaining: TimeInterval, hr: Int = 0, distance: Double = 0, steps: Int = 0) {
        guard let a = activity else { return }
        var s = a.contentState
        s.elapsed = elapsed
        s.remaining = remaining
        s.hr = hr
        s.distance = distance
        s.steps = steps
        let newState = s
        // Cancel any pending update before scheduling a new one
        updateTask?.cancel()
        updateTask = Task { await a.update(using: newState) }
    }

    func end() {
        updateTask?.cancel()
        updateTask = nil
        guard let a = activity else { return }
        activity = nil
        Task { await a.end(dismissalPolicy: .immediate) }
    }
}
